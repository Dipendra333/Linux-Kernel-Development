...
#include <linux/kfifo.h>    // For ring buffer
...
{  
    pr_info("Workqueue executed in the process context...\n");
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
    ...
    // Schedule the timer to fire once after 2 seconds
...
static ssize_t message_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
    snprintf(message, sizeof(message), "%.*s", (int) count, buf);
    return count;
}

static DEVICE_ATTR(message, 0664, message_show, message_store);
...
static int __init my_init(void)
{
    ...
    devs[i].device= device_create(my_class, NULL, dev_num, NULL, dev_name);
    device_create_file(devs[i].device, &dev_attr_message);
    ...
    INIT_WORK(&my_work, my_workqueue_handler);
...
static void __exit my_exit(void)
{
    ...
    for(i=0; i<MAX_MINOR; i++) {
    dev_num = MKDEV(MAJOR(my_device), MINOR(my_device) + i);
    device_remove_file(devs[i].device, &dev_attr_message);
...
MODULE_DESCRIPTION("Sample Workqueue Implementation");
