...
static ssize_t my_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ...
    pr_info("Waiting for data available...\n");
    wait_event_interruptible(data->wq, data->data_available); // waits for data->data_avialable to be true
    pr_info("Waking up after data is available..\n");
    ...
    *pos += count;
    ...
    return count;
}  /* cat before echo to test the sleeping of the process until the condition is stisfied
    cat /dev/my_device_name0   (-> results the process to sleep)
    echo "Dipendra Khadka" > /dev/my_device_name0 (-> it wakes up the process and that is posted )
*/
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
    ...
    data->data_available = true; // set data available 
